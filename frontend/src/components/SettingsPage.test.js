...
test('shows workspace defaults when model-bindings is empty but model-options includes default_bindings', async () => {
  global.fetch = jest.fn((url, options = {}) => {
    if (url.endsWith('/settings/input-sources') && (!options.method || options.method === 'GET')) {
      return buildJsonResponse([
        {
    ...
    if (url.endsWith('/settings/alert-rules')) {
      return buildJsonResponse([]);
    }
    ...
    if (url.endsWith('/settings/govee-trigger-endpoints')) {
      return buildJsonResponse([]);
    }
